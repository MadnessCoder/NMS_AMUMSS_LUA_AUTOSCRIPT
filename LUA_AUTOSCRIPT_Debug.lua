...
--====================================
 ...
 function CreateNewScript(Original_EXML_name,Modified_EXML_name)
	...
	while glb_CurrentLine ~= #glb_TextFileTableVanilla+1 do
			...
			-- setmetatable(glb_Tesei, {__index = Walker})
			-- printTable(Walker(glb_CurrentLine).getStartNodeIndex())
			-- printTable(Walker(glb_CurrentLine))
			-- print(glb_Tesei)
			-- printTable(glb_Tesei)
			-- print(glb_Tesei.self)
			-- printTable(glb_Tesei.getStartLineNum())
			-- printTable(glb_Tesei.getCurrentNodeIndex())
			glb_NodeList = nil -- Global!
			-- glb_NodeList[#glb_NodeList+1]=NullNode(nil, 0)
			DFS()
			glb_CurrentLine = glb_CurrentLine + glb_Tesei.getLastNodeIndex()
			-- print(glb_CurrentLine) -- Debug option
	end
...
function Walker(StartLineNum)
	...
	local function goRightAndDown()
		if glb_NodeList == nil then
			...
			-- print("HERE")
			-- printTable(glb_Tesei.self)
			-- printTable(glb_NodeList[glb_Tesei.getCurrentNodeIndex()])
			
			
			table.insert(glb_NodeList, Node(glb_Tesei.getCurrentNodeIndex(), 1))
			-- glb_NodeList[1]=Node(glb_Tesei.getCurrentNodeIndex(),NullNodeObj.getCounter_RightAndDown()+1)
			-- printTable(glb_NodeList[1])
	...
	local function goDownStraight()
		if glb_Tesei.getCurrentNodeIndex() ~= 0 and glb_Tesei.getCurrentNodeIndex() ~= 1 then -- to prevent DownStraight moving in null and first nodes. First node is pseudo-null node.
			-- if glb_Tesei.getCurrentNodeIndex() ~= 1 then
			-- print("HERE")
			-- printTable(glb_Tesei.getCurrentNodeIndex())
			-- printTable(glb_Tesei.self)
			-- printTable(glb_NodeList[glb_Tesei.getCurrentNodeIndex()])
			if glb_NodeList[glb_Tesei.getCurrentNodeIndex()].getFlag_DownStraight() == 0 then
				...
				if checkDownStraight() == true then
					...
					glb_Tesei.setCurrentNodeIndex(glb_Tesei.getLastNodeIndex()+1)
					glb_Tesei.increaseLastNodeIndex()
					-- print(glb_Tesei.getCurrentNodeIndex)
					if LinesDifferent(glb_Tesei.getStartLineNum()+glb_Tesei.getCurrentNodeIndex()) == true then
						glb_FoundKeysAndValuesTable = nil 
						glb_ConvertedKeysAndValuesTable = {}
						GraphRootSearch()
...
function checkRightAndDown()
	...
	-- print("HERE")
	-- printTable(glb_Tesei.getCurrentNodeIndex())
	-- printTable(glb_Tesei.getLastNodeIndex())
	-- printTable(glb_Tesei.getStartLineNum())
	-- sleep(1000)
...
function ExtractNameAndValueFromLine(LineNum)
	print(LineNum, glb_Tesei.getStartLineNum(), glb_Tesei.getPreviousVisitedNode()) -- Debug option	
	...
	if Start_pos1 ~= nil then
		...
		if Start_pos2 == nil then
			-- printTable(Output_string1) -- Debug option
		...
		table.insert(Output_string1_AND_Output_string2, Output_string2)
			-- print(Output_string1_AND_Output_string2) -- Debug options
			-- print(" ")
			-- printTable(Output_string1_AND_Output_string2)
			-- print(" ")
...
function TableInsertAndWordExtractor(TableWhereToInsert, IndexWhereToInsert, LineNum)
	...
	-- printTable(ExtractedValues) -- Debug option
...
function GraphRootSearch()
	...
	--====================================
	-- print("THIS")
	-- printTable(glb_NodeList[1].self)
	-- print(glb_Tesei.getCurrentNodeIndex())
	-- if #glb_NodeList > 1 then
	-- printTable(glb_NodeList[2].self)
	-- printTable(glb_NodeList[3].self)
	-- printTable(glb_NodeList[4].self)
	-- printTable(glb_NodeList[5].self)
	-- end
	
	-- for i = 1, glb_NodeList do
		-- printTable(glb_NodeList[i].self)
	-- end
	-- printTable(glb_NodeList[glb_Tesei.getPreviousVisitedNode()].getCounter_RightAndDown())
	-- printTable(glb_NodeList[glb_Tesei.getCurrentNodeIndex()].getFlag_DownStraight())
	-- print("THAT")
	--====================================
	...
	if	glb_Tesei.getCurrentNodeIndex() ~= 0 then
		...
		-- printTable(glb_FoundKeysAndValuesTable) -- Debug option
		TableConvertationToPKW(glb_FoundKeysAndValuesTable) -- Duct tape
		-- printTable(glb_ConvertedKeysAndValuesTable) -- Debug option
...
-- Debug helpers
--==================================================================================
-- I don't know why there is no such print function by default. It is necessary.
